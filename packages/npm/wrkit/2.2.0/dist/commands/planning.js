...
function formatDetails(event, start, end) {
    const startTs = Math.floor(start.getTime() / 1000);
    const endTs = Math.floor(end.getTime() / 1000);
    let details = `**${event.subject}**\n`;
    if (event.type)
        details += event.type === "Skillogs" ? `Sur : ${event.type}\n` : `Salle : ${event.type}\n`;
    if (event.teacher)
        details += `Professeur : ${event.teacher}\n`;
    if (event.classes?.filter((c) => c.trim()).length)
        details += `Classes : ${event.classes.join(", ")}\n`;
    details += `De : <t:${startTs}:t> à <t:${endTs}:t>\n`;
    return details;
}
function createButtons(page, total) {
...
export const command = {
    ...
    async execute(interaction, logger) {
        ...
        const planningUrl = speciality === "SLAM" ? encodeURIComponent(env.PLANNING_SLAM) : encodeURIComponent(env.PLANNING_SISR);
        try {
...
async function handleNext(interaction, semaine, speciality, maintenant, logger) {
...
async function handleCurrent(interaction, semaine, speciality, maintenant, logger) {
...
async function handleFull(interaction, semaine, speciality, maintenant, logger) {
    const eListe = semaine.map((e) => {
        ...
        if (e.type)
            details += e.type === "Skillogs" ? `Sur : ${e.type}\n` : `Salle : ${e.type}\n`;
        if (e.teacher)
            details += `Professeur : ${e.teacher}\n`;
        if (e.classes?.filter((c) => c.trim()).length)
            details += `Classes : ${e.classes.join(", ")}\n`;
    ...
    action.on("collect", async (btnInteraction) => {
        ...
        if (btnInteraction.customId === "precedent")
            currentPage--;
        else if (btnInteraction.customId === "suivant")
            currentPage++;
        try {
