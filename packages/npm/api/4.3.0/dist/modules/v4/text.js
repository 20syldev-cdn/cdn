...
export function stats(value) {
    ...
    for (const c of value.replace(/\s/g, '').toLowerCase())
...
export function lorem(type, count) {
...
function frBelowThousand(n) {
    ...
    if (h === 1)
        ...
        result = FR_UNITS[h] + ' cent' + (r === 0 ? 's' : '');
    if (r > 0)
        result += ' ' + frBelowHundred(r);
    return result;
}
function numberToFrench(n) {
    if (n === 0)
        return 'zéro';
    if (n < 0)
        return 'moins ' + numberToFrench(-n);
    if (n < 1000)
        return frBelowThousand(n);
    if (n < 1_000_000) {
