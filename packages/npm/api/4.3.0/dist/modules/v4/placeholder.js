function parseSize(value, name, def) {
    if (value === undefined)
        return def;
    ...
    if (n < 1 || n > 4000)
        throw new Error(`${name} must be between 1 and 4000`);
    return Math.floor(n);
}
function normalizeColor(value, def) {
    if (!value)
        return def;
...
function avatarShape(shape, x, y, size) {
    const r = size / 2;
    switch (shape) {
        case 'circle':
...
function animationDefs(mode, bg, highlight, speed) {
    if (mode === 'none')
        return '';
    if (mode === 'pulse') {
        return `<style>
...
function generateSkeleton(opts) {
    ...
    if (avatar) {
        ...
        if (count === 2) {
            ...
            shapes.push(`<rect x="${lx}" y="${nameY + lineH + gap}" width="${lw * 0.35}" height="${lineH}" rx="${rx}" />`);
        }
        else if (count === 1) {
...
function parseAnimate(value) {
    if (!value)
        return 'shimmer';
    if (ANIMATE_MODES.has(value))
        return value;
    throw new Error('animate must be one of: shimmer, pulse, none');
}
export default function placeholder(type, query) {
