...
function forfeitGame(params, games, sessions) {
...
function playMove(params, games, sessions, u, now) {
    ...
    if (!validMoves.includes(move))
        throw new Error('Invalid move. Please provide a valid move (e.g., 1-1, 2-2, 3-3).');
    if (sessions[u] && sessions[u].user !== session) {
        throw new Error('Session ID mismatch');
    }
    ...
    const play = { username: u, move, session };
    moves.push(play);
    g.last = now;
    ...
    if (result.winner || result.tie) {
        setTimeout(() => {
            if (games[game] && games[game].creation === creation)
                delete games[game];
        }, GAME_CLEANUP_TTL);
        return {
    ...
    sessions[u].last = now;
    setTimeout(() => {
        if (sessions[u] && Date.now() - sessions[u].last >= SESSION_TTL)
            delete sessions[u];
    }, SESSION_TTL);
...
function checkGame(moves) {
    ...
    const loser = winner && playersOrder.length === 2 ? (playersOrder.find((player) => player !== winner) ?? null) : null;
