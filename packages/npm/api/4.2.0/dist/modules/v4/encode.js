...
export function morse(value) {
    ...
    return value
        ...
        if (!code)
            throw new Error(`Unsupported character in Morse code: '${c}'`);
        return code;
    })
        .filter(Boolean)
        .join(' '))
...
export function unmorse(value) {
    ...
    return value
        ...
        if (!char)
            throw new Error(`Unsupported Morse sequence: '${code}'`);
        return char;
    })
        .join(''))
        .join(' ');
...
export function caesar(value, shift) {
    checkText(value);
...
export function unbinary(value) {
    checkText(value);
    if (!/^[01\s]+$/.test(value))
    ...
    return value
        ...
        .map((b) => {
        if (b.length !== 8)
            throw new Error('Each binary group must contain 8 bits');
