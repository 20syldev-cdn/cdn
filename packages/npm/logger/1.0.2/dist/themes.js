...
export const themes = {
    ...
    detailed: {
        format: "{time} {level} {icon} {method} {url} \u2192 {status} {duration}",
        colors: defaultColors,
        icons: defaultIcons,
    },
    plain: {
 ...
 * @returns The matching ANSI escape sequence, or an empty string
 */
export function getLevelColor(level, colors) {
    if (!colors)
        return "";
    switch (level) {
        case "debug":
...
export function formatEntry(entry, theme, groupName, timeFormat) {
    ...
    const level = findField(entry, ["level"]);
    ...
    let line = format
        ...
        .replace("{level}", c(getLevelColor(level, colors), level ? level.toUpperCase() : ""))
